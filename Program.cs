...
// Configure Data Protection to avoid DPAPI errors
builder.Services.AddDataProtection()
    .PersistKeysToFileSystem(new DirectoryInfo(Path.Combine(builder.Environment.ContentRootPath, "Keys")))
    .SetApplicationName("YogaCenterManagement");
...
//));
builder.Services.AddDbContext<YGCContext>(options =>
    options.UseSqlServer(builder.Configuration.GetConnectionString("YGC"), sqlOptions =>
        sqlOptions.EnableRetryOnFailure(
            maxRetryCount: 5,
            maxRetryDelay: TimeSpan.FromSeconds(30),
            errorNumbersToAdd: new int[] { -2, 53, 233, 10053, 10054, 10060, 40197, 40501, 40613, 49918, 49919, 49920, 11001 })));
